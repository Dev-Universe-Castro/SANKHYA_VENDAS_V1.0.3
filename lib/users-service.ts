...
export const usersService = {
  ...
  async create(userData: Omit<User, "id">): Promise<User> {
    ...
    try {
      const sql = `
        INSERT INTO AD_USUARIOSVENDAS (NOME, EMAIL, SENHA, FUNCAO, STATUS, AVATAR, CODVEND)
        VALUES (:nome, :email, :senha, :funcao, :status, :avatar, :codvend)
      `;
      ...
      await oracleService.executeQuery(sql, {
        ...
        avatar: avatarUrl,
        codvend: userData.codVendedor || null
      });

      console.log("✅ Usuário criado no Oracle");
  ...
  async update(id: number, userData: Partial<User>): Promise<User | null> {
    ...
    try {
      ...
      const binds: any = {
        ...
        avatar: avatarUrl,
        codvend: mergedData.codVendedor,
        id: id
      };
      ...
      console.log("📤 Executando atualização SQL");
