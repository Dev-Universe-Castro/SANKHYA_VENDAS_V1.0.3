  ...
  try {
    ...
    const leadsQuery = `
      ...
      WHERE l.ID_EMPRESA = :idEmpresa
        ...
        ${!ehAdmin && usuarioId ? 'AND l.CODUSUARIO = :codUsuario' : ''}
      ORDER BY l.DATA_CRIACAO DESC
    `;

    const leadsParams: any = { idEmpresa: empresaId };
    if (dataInicio) leadsParams.dataInicio = dataInicio;
    if (dataFim) leadsParams.dataFim = dataFim;
    if (!ehAdmin && usuarioId) leadsParams.codUsuario = usuarioId;
    ...
    const produtosLeadsQuery = `
      SELECT 
        ...
        pl.VLRUNIT,
        pl.VLRTOTAL,
        TO_CHAR(pl.DATA_INCLUSAO, 'DD/MM/YYYY') AS DATA_INCLUSAO
      FROM AD_ADLEADSPRODUTOS pl
      WHERE pl.ID_EMPRESA = :idEmpresa
        AND pl.ATIVO = 'S'
    `;

    const produtosLeads = await oracleService.executeQuery(produtosLeadsQuery, { idEmpresa: empresaId });

    // 3. ATIVIDADES - Tabela AD_ADLEADSATIVIDADES
    ...
    const atividadesParams: any = { idEmpresa: empresaId };
    ...
    // 4. PEDIDOS - Tabela AS_CABECALHO_NOTA
    const pedidosQuery = `
      SELECT 
        cab.NUNOTA,
        cab.ID_SISTEMA,
      ...
      WHERE cab.ID_SISTEMA = :idEmpresa
        ...
        ${!ehAdmin && usuarioId ? 'AND cab.CODVEND IN (SELECT CODVEND FROM AD_USUARIOSVENDAS WHERE CODUSUARIO = :codUsuario)' : ''}
      ORDER BY cab.DTNEG DESC
    `;

    const pedidosParams: any = { idEmpresa: empresaId };
    if (dataInicio) pedidosParams.dataInicio = dataInicio;
    if (dataFim) pedidosParams.dataFim = dataFim;
    if (!ehAdmin && usuarioId) pedidosParams.codUsuario = usuarioId;
    ...
    // 6. CLIENTES/PARCEIROS - Tabela AS_PARCEIROS
    const clientesQuery = `
      SELECT 
        ID_SISTEMA,
        CODPARC,
    ...
    let clientes = [];
    try {
      clientes = await oracleService.executeQuery(clientesQuery, { idEmpresa: empresaId });
    } catch (error: any) {
      console.warn('⚠️ Erro ao buscar clientes:', error.message);
    }

    // 7. FINANCEIRO - Tabela AS_FINANCEIRO
    ...
    const financeiroParams: any = { idEmpresa };
    ...
    // 9. ESTÁGIOS DOS FUNIS - Tabela AD_FUNISESTAGIOS
